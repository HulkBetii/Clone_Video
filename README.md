...
- Automatic Japanese and Korean captions are bypassed in favor of audio-first local Whisper transcription with a forced `ja` or `ko` hint, VAD, and word timestamps. If audio/model transcription fails, the job fails safely instead of returning the caption track.
- Japanese and Korean auto-caption tracks are retained temporarily as timestamped references. Low-confidence or materially different Whisper spans are rechecked with the configured `large-v3` model; only caption + secondary-model consensus can correct text. Ambiguous spans keep primary Whisper text and are recorded in JSON reconciliation audit metadata.
- Reconciliation aligns caption characters to Whisper word timestamps in monotonic 60-second windows with 5-second overlap, applies Japanese Kana or Korean Hangul-aware normalization only to alignment data, and prioritizes low-confidence spans. Japanese is bounded to 60 windows; Korean is bounded to 100 windows; both remain limited to 10 minutes of secondary audio per job.
- Korean Tier 1 spans can use whitespace-only surface mapping and a `0.01` timestamp-overlap tolerance when caption and `large-v3` agree exactly. Tier 2/3, partial secondary words, neighbor leakage, and non-whitespace mismatches retain the primary Whisper text.
- Reconciliation artifacts use schema version `3`; transcript cache keys use pipeline version `6` and include the secondary model profile. Selected and successfully processed secondary windows are reported separately from budget-skipped spans.
- Without a requested language, missing captions trigger local Whisper transcription. Automatic captions for languages other than Japanese and Korean keep the caption-first behavior.
