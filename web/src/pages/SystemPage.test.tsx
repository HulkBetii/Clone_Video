...
it("locks all GPT runtime actions that require an idle browser while busy", async () => {
  server.use(
    http.get("/api/v1/health", () => HttpResponse.json({ status: "ok", checks: { sqlite: true, yt_dlp: "1.0", ffmpeg: true, ffprobe: true, whisper: { device: "cpu" }, gpt_rewrite: { profile_id: "PROFILE_GPT_1" } } })),
...
it("explains lazy Whisper loading and CPU fallback when CUDA userspace is missing", async () => {
  server.use(
    http.get("/api/v1/health", () => HttpResponse.json({ status: "degraded", checks: { sqlite: true, yt_dlp: "1.0", ffmpeg: true, ffprobe: true, whisper: { model: "turbo", loaded: false, device: "not_loaded", compute_type: "not_loaded", cuda_device_count: 1, cuda_runtime_available: false, runtime_error: "cublas64_12.dll_NOT_FOUND" }, gpt_rewrite: { profile_id: "PROFILE_GPT_1" } } })),
